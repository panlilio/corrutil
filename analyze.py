...
class Reader:
    def __init__(self, dirname=None, regexpression=r"(?P<sample>\w+)_(?P<fov>[.\w]+)_ch(?P<channel>\d+)\S*.tif"):
        self.dirname = dirname
        self.regexpression = re.compile(regexpression)
        self.groups = {grp : [] for grp in self.regexpression.groupindex.keys()}
        self.filedict = {}
        for f in sorted(os.listdir(self.dirname)):
            m = self.regexpression.match(f)
            if m is not None:
                self.filedict[f] = m.groupdict()
                self.filedict[f]['channel'] = int(self.filedict[f]['channel'])
                for grp,val in self.filedict[f].items():
                    if val not in self.groups[grp]:
                        self.groups[grp].append(val)

        self.groups['channel'] = [int(ch) for ch in self.groups['channel']]
        self.c0 = np.min(self.groups['channel'])
        self.nc = np.max(self.groups['channel'])- self.c0 + 1
        

    def get_dataset(self,filter=None,**dataset_kwargs):
        if filter is None:
        #Retrieve all images at all fields of view
            name = self.dirname
            passfun = lambda x : True
        else:
            name = "_".join([val for val in filter.values()])
            passfun = lambda x : np.all([self.filedict[x][key]==val for key,val in filter.items()])
        
        #Filter images
        files = list(self.filedict.keys())
        dicts = list(self.filedict.values())
        images = []
        while len(files) > 0:
            if passfun(files[0]):
                f = files[0]
                d = dicts[0]
                im_ = io.imread(os.path.join(self.dirname,files[0]))
                im = np.zeros((self.nc,im_.shape[0],im_.shape[1]))                
                im[:] = np.nan
                im[d['channel']-self.c0] = im_
                files.remove(f)
                dicts.remove(d)
                f2 = copy.copy(files)
                d2 = copy.copy(dicts)
                for f_,d_ in zip(f2,d2):                                     
                    getme = True
                    for grp,val in d.items():
                        if grp!='channel':
                            getme = getme and d_[grp]==val
                    if getme:
                        im[d_['channel']-self.c0] = io.imread(os.path.join(self.dirname,f_))
                        files.remove(f_)
                        dicts.remove(d_)
                images.append(im)
            else:
    ...
    def generate_filter_list(self):
        ...
        for grp in self.groups.keys():
            if grp=="channel":
                ...
                F = self.append_filter(grp,self.groups[grp],F)
        return F

    @staticmethod
    def append_filter(grp,vals,f=[]):
        if len(f)==0:
            for v in vals:
                f.append({ grp : v })
        else:
...
def main(dirname,regexpression=r"(?P<sample>\w+)_(?P<fov>[.\w]+)_ch(?P<channel>\d+)\S*.tif",
    ...
    reader = Reader(dirname=dirname,regexpression=regexpression)
    writer = Writer(destdir=destdir, stats_to_get=stats_to_get)
    datafilters = reader.generate_filter_list()
    datastore = None
    csv_files = [] 
    for i,df in enumerate(datafilters):
        dset = reader.get_dataset(filter=df, do_bootstrap=do_bootstrap, n_bootstraps=n_bootstraps, bootstrap_block_size=bootstrap_block_size,
                                    nn=nn, discrete=discrete, CI=CI, threshold=threshold, threshold_kw=threshold_kw, area_min=area_min)
        logger.info(f"({i+1}/{len(datafilters)}) Dataset = {dset.name}. PROCESSING...")
